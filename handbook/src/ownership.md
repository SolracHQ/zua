...
```zig
fn makeTable(ctx: *zua.Context) !zua.Handlers.Any.Table {
...
When a `zua.Handlers.Any.Function` or `zua.Handlers.Any.Table` arrives as a function parameter, it is borrowed. Use it directly if you only need it during the call. If you need it later, take ownership:

```zig
fn processTable(ctx: *zua.Context, t: zua.Handlers.Any.Table) !i32 {
    // t is borrowed, valid only during this call
    return try t.get(ctx, "value", i32);
}

fn storeTable(_: *zua.Context, t: zua.Handlers.Any.Table) void {
...
| Type | Wraps | Notes |
|---|---|---|
| `zua.Handlers.Any.Table` | Lua table | stack or registry owned |
| `zua.Handlers.Any.Function` | Lua function | stack or registry owned |
| `zua.Handlers.Any.Userdata` | Lua full userdata | raw untyped handle |
| `zua.Handlers.Typed.Object(T)` | Lua full userdata | typed wrapper over `Userdata`, `.get()` returns `*T` |
| `zua.Handlers.Typed.Fn(ins, outs)` | Lua function | typed wrapper over `Function` |
| `zua.Handlers.Typed.TableView(T)` | Lua table | typed mirror with sync back to Lua |

`zua.Handlers.Typed.Object(T)` and `zua.Handlers.Typed.Fn(ins, outs)` are covered in their own chapters. The ownership rules are the same for all handle types: borrowed parameters are valid only during the call, and registry-owned handles must be released.
