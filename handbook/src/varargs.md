...
Sometimes you want a function that accepts any number of Lua arguments without fixing the arity upfront. Declare `zua.Mapper.Decoder.VarArgs` as the **last** parameter and zua collects every remaining Lua argument into a `[]Primitive` slice allocated from the context arena.
...
```zig
fn log(prefix: []const u8, rest: zua.Mapper.Decoder.VarArgs) void {
    for (rest.args) |arg| {
        std.debug.print("{s}: {s}\n", .{ prefix, @tagName(arg) });
    }
}
...
```zig
fn firstOrDefault(ctx: *zua.Context, args: zua.Mapper.Decoder.VarArgs) !i32 {
