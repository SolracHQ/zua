//! Encodes Zig values into Lua and decodes Lua values back into Zig. This is the central translation layer. `Encoder.push`
//! converts a Zig value to the Lua stack, `Decoder.pop` reads it back. The translation strategy for each type comes from
//! its `ZUA_SHAPE` metadata.
const Mapper = @This();
...
/// Lua primitive type tag. Mirrors the `Primitive` union variants without carrying any payload. Useful for error reporting
/// and type dispatch.
...
///
/// Represents a Lua value after type-checking but before type-specific decoding. The `table` variant holds a borrowed
...
///
/// Declare `VarArgs` as the last parameter of a callback to receive all Lua arguments that were not matched by preceding
/// parameters. The slice is allocated from the context arena and is valid for the duration of the call.
///
/// Example:
/// ```zig
