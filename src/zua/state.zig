//! Fat wrapper over a Lua VM instance. Owns the `lua_State`, allocator, I/O interface, and metatable cache. Lives as long
//! as the Lua state lives, stored as Lua userdata with `__gc` for automatic cleanup.
//!
//! Create one with `init` for a fresh Lua environment or `libState` when you receive an existing `lua_State*` from a host
//! that already has one. The pointer returned by either path is stable and safe to capture in callbacks. Recover it from a
//! raw `lua_State*` with `fromState`.
...
luaState: *lua.State,
/// Each metatable is keyed by `@typeName(T)` so that types with the same name in different modules share a cached
/// metatable. References are stored in the Lua registry and unrefed on cleanup.
...
/// Creates a Lua-managed ZuaState userdata, opens Lua standard libraries, and stores the pointer in the registry. The
/// returned pointer is stable and safe to capture in callbacks as long as the Lua state remains alive.
...
/// Creates or reuses a ZuaState inside an existing `lua_State*`. Uses a suffix-based registry key so dylib reloads can
...
/// Closes the Lua state. Zua-owned runtime state is cleaned up when Lua collects the state userdata.
pub fn deinit(self: *State) void {
    lua.deinit(self.luaState);
}

/// Pushes the metatable for T onto the stack. On first call the metatable is built from `T`'s `ZUA_SHAPE` metadata:
/// declared methods and metamethods are wired through trampolines and cached by `@typeName(T)`.
...
/// Borrowed handle to the Lua globals table (`_G`). Each call pushes a value onto the Lua stack that must be released or
/// balanced. Use `addGlobals` to write into it without managing the stack yourself.
...
///
/// This is equivalent to `Mapper.Encoder.fillTable` on `state.globals()` and can be called multiple times to register
...
/// Borrowed handle to the Lua registry. Each call pushes a value onto the Lua stack that must be released or balanced. Use
/// this to store host state via `setLightUserdata("key", &state)`.
...
///
/// Use with `defer state.popTop()` around code that may push/pop values, mirroring the Lua C API pattern of saving and
/// restoring the stack. In debug builds, the call site is captured to detect unbalanced usage.
...
/// Recovers the owning ZuaState instance from a raw `lua_State` pointer. Called by the callback trampoline to retrieve the
/// ZuaState context.
