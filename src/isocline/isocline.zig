...
//!
//! This module keeps the original isocline semantics, but exposes Zig slice types, boolean results, and documented aliases
//! so the API feels natural from Zig.
...
///
/// The returned slice is borrowed from the NUL-terminated C string returned by isocline and is only valid until the
/// returned value is freed using `freeMemory`.
...
///
/// The returned slice is borrowed from the NUL-terminated C string returned by isocline and is only valid until the
/// returned value is freed using `freeMemory`. Returns `null` on error, or if the user typed ctrl+d or ctrl+c.
...
///
/// Both callbacks can be `null` in which case the defaults are used. The returned slice is borrowed from the NUL-terminated
/// C string and is only valid until it is freed with `freeMemory`.
...
///
/// There can only be one default completion function. Setting it again disables the previous one. The initial completer is
...
///
/// The completion string is copied by isocline and does not need to be preserved or allocated by the caller. Returns `true`
/// to continue trying to find more completions. Returns `false` when the callback should stop adding completions.
...
///
/// The `display` is used to show the completion in the menu, and `help` is displayed for hints. Both can be `null` for the
/// default. Returns `true` to continue trying to find more completions. Returns `false` when the callback should stop.
...
///
/// The `completions` array must be terminated with `null`. All elements are added if they start with `prefix`. Returns
/// `true` to continue trying to add completions. Returns `false` when the callback should stop.
...
///
/// If `roots` is `null`, the current directory is used. If `extensions` is `null`, any extension will match. If a directory
/// is completed, the `dir_separator` is appended when it is not `0`.
...
///
/// Pass `null` for `prompt_marker` to use the default "> ". Pass `null` for `continuation_prompt_marker` to have it match
/// `prompt_marker`.
...
/// Disable or enable multi-line input (enabled by default). Returns the previous setting.
pub fn enableMultiline(enable: bool) bool {
    return isocline_c.ic_enable_multiline(enable);
}

/// Disable or enable sound (enabled by default). A beep is used when tab cannot find any completion for example. Returns
/// the previous setting.
pub fn enableBeep(enable: bool) bool {
    return isocline_c.ic_enable_beep(enable);
}

/// Disable or enable color output (enabled by default). Returns the previous setting.
pub fn enableColor(enable: bool) bool {
    return isocline_c.ic_enable_color(enable);
}

/// Disable or enable duplicate entries in the history (disabled by default). Returns the previous setting.
pub fn enableHistoryDuplicates(enable: bool) bool {
    return isocline_c.ic_enable_history_duplicates(enable);
}

/// Disable or enable automatic tab completion after a completion to expand as far as possible if the completions are
/// unique. Returns the previous setting.
...
/// Disable or enable automatic indentation of continuation lines in multiline input so it aligns with the initial prompt.
/// Returns the previous setting.
pub fn enableMultilineIndent(enable: bool) bool {
    return isocline_c.ic_enable_multiline_indent(enable);
}

/// Disable or enable display of short help messages for history search etc. Full help is always displayed when pressing F1
/// regardless of this setting. Returns the previous setting.
pub fn enableInlineHelp(enable: bool) bool {
    return isocline_c.ic_enable_inline_help(enable);
}

/// Disable or enable hinting (enabled by default). Shows a hint inline when there is a single possible completion. Returns
/// the previous setting.
...
/// Disable or enable syntax highlighting (enabled by default). This applies regardless whether a syntax highlighter
/// callback was set. Returns the previous setting.
pub fn enableHighlight(enable: bool) bool {
    return isocline_c.ic_enable_highlight(enable);
}

/// Set millisecond delay for reading escape sequences in order to distinguish a lone ESC from the start of an escape
/// sequence. The defaults are 100ms and 10ms.
...
///
/// There can only be one highlight function. Setting a new one disables the previous one.
...
/// Enable or disable automatic brace insertion (enabled by default). Returns the previous setting.
...
///
/// Calls `fun` on the current word and adjusts completion results to replace that part of the input. If `is_word_char` is
/// `null`, the default `ic_char_is_nonseparator` is used.
...
///
/// Takes quotes and escape characters into account when completing. If `is_word_char` is `null`, the default
/// `ic_char_is_nonseparator` is used.
...
///
/// Cannot be used with most completion transformers such as `completeWord`. `delete_before` and `delete_after` specify the
/// number of bytes to remove before and after the cursor before inserting `completion`.
